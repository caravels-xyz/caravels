...
from caravels.llm import StubProvider
...
def _snapshot() -> MarketSnapshot:
    return MarketSnapshot(
        tokens={
            "ETH": TokenFeatures(token="ETH", price_usd=2000.0, rsi_14=45.0, macd=5.0, macd_signal=3.0, ema_20=1950.0, fear_greed=40.0, price_change_24h_pct=2.0),
            "AVAX": TokenFeatures(token="AVAX", price_usd=25.0, rsi_14=30.0, macd=0.5, macd_signal=0.3, ema_20=24.0, fear_greed=40.0, price_change_24h_pct=3.0),
            "LINK": TokenFeatures(token="LINK", price_usd=10.0, rsi_14=35.0, macd=0.2, macd_signal=0.1, ema_20=9.8, fear_greed=40.0, price_change_24h_pct=1.5),
        },
...
def _cfg(**kwargs) -> AppConfig:
    defaults = dict(dry_run=True, competition_mode=False, emergency_pause=False, ladder_enabled=False, llm_provider="stub", helm_max_actions_per_tick=2)
...
class TestGenerateReturnsList:
    def test_returns_list_of_candidates(self, healthy_score):
        ...
        result = helm_signal.generate(_snapshot(), cfg, stub, portfolio=_portfolio(), competition=_competition(), score=healthy_score)
    ...
    def test_single_hold_still_returns_1_element_list(self, healthy_score):
        ...
        stub = StubProvider()
        cfg = _cfg(helm_agentic=False)
        candidates, _ = helm_signal.generate(_snapshot(), cfg, stub, portfolio=_portfolio(), competition=_competition(), score=healthy_score)
        assert len(candidates) >= 1
